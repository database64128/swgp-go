		...
		for i := range msgvecn {
			...
			if !ok {
				...
				go func() {
					...
					wgAddress := s.wgAddr.String()

					wgConn, wgConnInfo, err := s.wgConnConfig.DialMmsgConn(ctx, s.wgConnListenAddress, s.wgNetwork, wgAddress)
					if err != nil {
						logger.Warn("Failed to create UDP socket for new session",
							tslog.AddrPort("clientAddress", clientAddrPort),
							tslog.ConnAddr("wgConnListenAddress", s.wgConnListenAddress),
							slog.String("wgNetwork", s.wgNetwork),
							slog.String("wgAddress", wgAddress),
							tslog.Err(err),
						)
						return
					}
...
func (s *server) relayProxyToWgSendmmsg(
	...
	dequeue:
		...
		for start := 0; start < len(msgvec); {
			n, err := wgConn.WriteMsgs(msgvec[start:], 0)
			start += n
			if err != nil {
...
func (s *server) relayWgToProxySendmmsg(
	...
	recvPacketBuf := make([]byte, s.relayBatchSize*maxProxyPacketSize)
	recvCmsgBuf := make([]byte, s.relayBatchSize*conn.SocketControlMessageBufferSize)
	sendPacketBuf := make([]byte, 0, s.relayBatchSize*maxProxyPacketSize)
	...
	for {
		nr, err := wgConn.ReadMsgs(rmsgvec, 0)
		if err != nil {
			if errors.Is(err, os.ErrDeadlineExceeded) {
				break
			}
		...
		for start := 0; start < len(smsgvec); {
			n, err := proxyConn.WriteMsgs(smsgvec[start:], 0)
			start += n
			if err != nil {
