...
func (c *client) recvFromWgConnRecvmmsg(ctx context.Context, logger *tslog.Logger, wgConn *conn.MmsgRConn, wgConnInfo conn.SocketInfo) {
	...
	for {
		...
		for i := range msgvecn {
			...
			if err != nil {
				logger.Error("Failed to parse sockaddr of packet from wgConn", tslog.Err(err))
				continue
			}
			...
			if !ok {
				...
				c.wg.Go(func() {
					...
					proxyAddress := c.proxyAddr.String()

					proxyConn, proxyConnInfo, err := c.proxyConnConfig.DialMmsgConn(ctx, c.proxyConnListenAddress, c.proxyNetwork, proxyAddress)
					if err != nil {
						logger.Warn("Failed to create UDP socket for new session",
							tslog.AddrPort("clientAddress", clientAddrPort),
							tslog.ConnAddr("proxyConnListenAddress", c.proxyConnListenAddress),
							slog.String("proxyNetwork", c.proxyNetwork),
							slog.String("proxyAddress", proxyAddress),
							tslog.Err(err),
						)
						return
					}
			...
			default:
				if logger.Enabled(slog.LevelDebug) {
					logger.Debug("swgpPacket dropped: send channel is full",
						tslog.AddrPort("clientAddress", clientAddrPort),
						tslog.ConnAddrp("proxyAddress", &c.proxyAddr),
					)
				}
...
main:
	...
	dequeue:
		...
		for start := 0; start < len(msgvec); {
			n, err := proxyConn.WriteMsgs(msgvec[start:], 0)
			start += n
			if err != nil {
...
func (c *client) relayProxyToWgSendmmsg(
	...
	recvPacketBuf := make([]byte, c.relayBatchSize*maxProxyPacketSize)
	recvCmsgBuf := make([]byte, c.relayBatchSize*conn.SocketControlMessageBufferSize)
	sendPacketBuf := make([]byte, 0, c.relayBatchSize*maxProxyPacketSize)
	...
	for i := range c.relayBatchSize {
		...
		riovec[i].SetLen(maxProxyPacketSize)
		...
		recvPacketBufp = unsafe.Add(recvPacketBufp, maxProxyPacketSize)
		recvCmsgBufp = unsafe.Add(recvCmsgBufp, conn.SocketControlMessageBufferSize)
	}

	for {
		nr, err := proxyConn.ReadMsgs(rmsgvec, 0)
		if err != nil {
			if errors.Is(err, os.ErrDeadlineExceeded) {
				break
			}
		...
		for i := range rmsgvecn {
			...
			remainingRecvPacketBuf = remainingRecvPacketBuf[maxProxyPacketSize:]
		...
		for start := 0; start < len(smsgvec); {
			n, err := wgConn.WriteMsgs(smsgvec[start:], 0)
			start += n
			if err != nil {
