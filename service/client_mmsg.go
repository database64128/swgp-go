		...
		for i := range msgvecn {
			...
			if !ok {
				...
				go func() {
					...
					proxyConn, proxyConnInfo, err := c.proxyConnConfig.ListenMmsgConn(ctx, proxyConnListenNetwork, c.proxyConnListenAddress)
					if err != nil {
...
func (c *client) relayProxyToWgSendmmsg(downlink clientNatDownlinkMmsg) {
	...
	for {
		...
		for i := range rmsgvecn {
			...
			if err != nil {
				downlink.logger.Error("Failed to parse sockaddr of packet from proxyConn", tslog.Err(err))
				continue
			}
